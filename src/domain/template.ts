...
const nodes: GraphNode[] = [
  ...
  { id: 'draft_brief', name: 'Draft brief', category: 'transform', description: 'Create a readable research brief from the evidence summary.', reads: ['evidenceSummary', 'researchGoal'], writes: ['draftBrief'], sideEffect: 'none', impact: 'low', position: { x: X + 200, y: 850 } },
  { id: 'human_review', name: 'Human review checkpoint', category: 'human_review', description: 'A person reads the draft and approves it, requests a revision, asks for more evidence, or stops the workflow.', reads: ['draftBrief', 'coverageScore', 'coverageGaps', 'errors'], writes: ['reviewerDecision', 'reviewerNotes'], sideEffect: 'none', impact: 'low', position: { x: X, y: 960 },
    oversight: { authority: 'approve_before', who: 'Research lead', sees: ['draftBrief', 'coverageScore', 'coverageGaps', 'iterationCount', 'errors'], actions: ['approve', 'revise', 'request_evidence', 'stop'] } },
...
const routers: Router[] = [
  {
    ...
    defaultTargetNodeId: 'improve_plan', defaultLabel: 'No, improve the plan', insufficientEvidenceTargetNodeId: 'human_review', maxIterations: 2,
    position: { x: X, y: 720 },
  },
  {
...
const edges: GraphEdge[] = [
  ...
  e('r_coverage', 'human_review', 'default', { label: '? no score', routerRuleId: 'insufficient' }),
...
const scenarios: TestScenario[] = [
  ...
  {
    id: 'evidence_unavailable', name: 'Coverage score unavailable',
    description: 'The coverage evaluator returns no score at all. Rather than guess, the coverage decision abstains and hands the case to the human checkpoint with the gaps it can see.',
    input: { researchGoal: goal },
    expectedRouterOutcomes: ['Insufficient evidence', 'Revision requested', 'Approved'],
    mockedOutcomes: {
      plan_tasks: { patches: [{ researchPlan: plan1 }] },
      research_a: { patches: [{ findings: findingsA }] },
      research_b: { patches: [{ findings: findingsB }] },
      research_c: { patches: [{ findings: findingsC }] },
      evaluate: { patches: [{ coverageScore: null, coverageGaps: ['Evaluator returned no score (mock outage)'] }] },
      human_review: { patches: [{ reviewerDecision: 'revise', reviewerNotes: 'No coverage score and no draft yet. Write the draft from the raw findings and resubmit.' }, { reviewerDecision: 'approved', reviewerNotes: 'Draft is acceptable. Note the evaluator outage in the brief.' }] },
      improve_draft: { patches: [{ draftBrief: 'Graph-based workflow tools... (mock draft written from the raw findings, 590 words)', iterationCount: 1 }] },
      combine: commonTail.combine,
      publish: commonTail.publish,
    },
  },
];
